...
def gibbs_removal_1d_fourier(x, a=0, fn=0, nn=3):
...
def gibbs_removal_2d(image, fn=0, nn=3, G0=None, G1=None, fourier=True):
    ...
    fourier : bool, optional
        If fourier is True image subshifts are done in fourier space. Otherwise
        shifts are done directly in image space using local linear
        interpolation
    ...
    if fourier:
        img_c1, tv_c1 =  gibbs_removal_1d_fourier(image, a=1, fn=fn, nn=nn)
        img_c0, tv_c0 =  gibbs_removal_1d_fourier(image, a=0, fn=fn, nn=nn)
    else:
...
def volume_gibbs_removal(vol, fn=0, nn=3, fourier=True):
