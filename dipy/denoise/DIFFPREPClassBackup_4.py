...
from dipy.align.Register_DWIs import register_dwi_to_b0, set_image_in_scale, get_gradients_params, get_direction_and_spacings
...
class diffprep(object):
    def __init__(self, input_image_path,
        ...
        if fov:
            ...
            self.fov = self.input_resolution*self.input_shape[:3]
...
#------------DIFFPREP - run all----------------------------#
    def execute(self):
        ...
        if self.activeDenoising:
            ...
            self.current_process_data = output_arr
        # if save_to_file:
            self.save_output_tofile(output_arr,
                                    output_name=self.output_image_fn.split('.nii')[0] + "_Denoised.nii")

        # Gibb Removing
        if self.activeGibbRemoving:
            print("----------Gibb Removal ----------------------")
            output_arr = self.get_GibbRemoved_data(self.current_process_data)
            self.current_process_data = output_arr
            self.save_output_tofile(output_arr,
                                    output_name=self.output_image_fn.split('.nii')[0] + "_GR.nii")
        ...
        if self.Eddy:
            DMC_image = self.dmc_make_target(self.b0, self.mask_mask_data)
            print("Perform Registration......")
            self.save_output_tofile(DMC_image,
                                    output_name=self.output_image_fn.split('.nii')[0] + "_DMC.nii")
...
########### GETTING SUB_FUNCTIONS ####################
    ...
    def get_Denoising_data(self, input_arr):
    ...
    def dmc_make_target(self, b0_image, b0_bi_mask):
        ...
        if np.sum(sz) < 0.3*np.sum(transformed_mask.shape):
            start = np.zeros(3)
            sz = transformed_mask.shape

        final_image = transformed_b0[start[0]: start[0] + sz[0],
                      start[1]: start[1] + sz[1],
                      start[2]: start[2] + sz[2]]

        msk_cnt = len(b0_bi_mask[b0_bi_mask != 0])
        npixel = b0_bi_mask.size

        # Set the equal to 1 - Keep the orginal image
        if msk_cnt < 0.02*npixel:
            b0_bi_mask[:] = 1
            # final_image = b0_bi_mask * transformed_b0
        return final_image
    ...
    def multi_sub_(self, curr_vol, b0_target, b0_target_bin, correction_mode = False):
        ...
        if not correction_mode:
            id_trans = QuadraticMap(self.phase_encoding)
            # id_trans.Setting_Identity()
            curr_trans = id_trans.get_QuadraticParams()

        else:
            curr_trans = self.RegisterDWI_to_B0(b0_target, curr_vol,self.phase_encoding, signal_ranges, optimizer_setting = True)
        return curr_trans


    # TODO: EDDY CURRENT DISTORTION
    def EDDY_Current_Distortion(self, b0_target):
